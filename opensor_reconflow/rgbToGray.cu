...
void sor::CudaFlow::rgbToGray(uchar3 * d_iRgb, float *d_iGray, int w, int h, int s)
{
...
void sor::CudaFlow::Cv8uToGray(uchar * d_iCv8u, float *d_iGray, int w, int h, int s)
{
	dim3 threads(BlockWidth, BlockHeight);
	dim3 blocks(iDivUp(w, threads.x), iDivUp(h, threads.y));
...
void sor::CudaFlow::Cv16uToGray(ushort * d_iCv16u, float *d_iGray, int w, int h, int s)
{
	dim3 threads(BlockWidth, BlockHeight);
	dim3 blocks(iDivUp(w, threads.x), iDivUp(h, threads.y));
...
void sor::CudaFlow::Cv16uTo32f(ushort * d_iCv16u, float *d_iGray, int w, int h, int s)
{
	dim3 threads(BlockWidth, BlockHeight);
	dim3 blocks(iDivUp(w, threads.x), iDivUp(h, threads.y));
...
void sor::CudaFlow::Cv32fToGray(float * d_iCv32f, float *d_iGray, int w, int h, int s)
{
	dim3 threads(BlockWidth, BlockHeight);
	dim3 blocks(iDivUp(w, threads.x), iDivUp(h, threads.y));
