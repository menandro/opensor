...
int sor::CudaFlow::copyMaskToDevice(cv::Mat mask0, cv::Mat mask1) {
...
int sor::CudaFlow::solveInpainting(float flowScale) {
	return _solveInpainting(flowScale);
}

int sor::CudaFlow::_solveInpainting(float flowScale) {
	// Convert RGB to Gray
