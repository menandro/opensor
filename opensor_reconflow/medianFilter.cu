...
__global__ void MedianFilterKernel5 (float* u, float* v, 
...
__global__ void MedianFilterKernel3 (float* u, float* v, 
	int width, int height, int stride,
	float *outputu, float *outputv)
{
	const int ix = threadIdx.x + blockIdx.x * blockDim.x;
...
void sor::CudaFlow::MedianFilter(float *inputu, float *inputv,
	int w, int h, int s,
	float *outputu, float*outputv,
	int kernelsize)
{
	...
	if (kernelsize == 3){
		MedianFilterKernel3 << < blocks, threads >> > (inputu, inputv,
			w, h, s, outputu, outputv);
	}
	else if (kernelsize == 5) {
		MedianFilterKernel5 << < blocks, threads >> > (inputu, inputv, 
			w, h, s, outputu, outputv);
	}
}
...
//*************************************
__global__ void MedianFilter3DKernel3(float* X, float* Y, float *Z,
	int width, int height, int stride,
...
void sor::CudaFlow::MedianFilter3D(float *X, float *Y, float *Z,
	int w, int h, int s,
	float *X1, float *Y1, float *Z1,
	int kernelsize)
{
	...
	if (kernelsize == 3) {
		MedianFilter3DKernel3 << < blocks, threads >> > (X, Y, Z,
			w, h, s, X1, Y1, Z1);
	}
}
