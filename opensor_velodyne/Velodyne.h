...
namespace sor {
	...
	class Velodyne {
	public:
		// Reading UDP/TCP broadcast
		Velodyne(const char *address_str, unsigned short port, CaptureMode captureMode);
		...
		boost::asio::ip::tcp::socket* tcpsocket = nullptr;
		...
		bool openTCP(boost::asio::ip::address& address, const unsigned short port);
